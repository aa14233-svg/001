...
这一卷横跨 7 大类工具——VS Code（Cline/Continue）、QwenCoder、OpenCode、Claude Code、Trae（MetaKing Agent IDE）、Hermes、以及其他工具（Cursor/Windsurf/Aider/JetBrains）——系统讲解在每个工具中如何设计原创智能体的 System Prompt、Skill 路由和 MCP 集成。
...
- → MetaKing-Skill 系统（原创智能体的顶层路由参照，L0→L3 路由层级可适配至各工具）
...
**Skill 路由：** OpenCode 实现了基于正则/关键词匹配的轻量路由机制，其思想与 MetaKing-Skill 的 L1 特征词典一致：
...
## 2.5 Trae（MetaKing Agent IDE）原创智能体设计

Trae（MetaKing Agent IDE）是一个以 Agent 为核心的一体化开发环境。其 MetaKing Agent 架构已在 MetaKing-Skill 文件中详细阐述，核心要点：MetaKing Agent 负责全局任务编排，SkillHub 管理 Skill 注册，四层路由（L0→L3）实现智能路由，cc-switch 管理 AI Provider 切换与 MCP 工具。
...
| Trae (MetaKing Agent) | JSON | 通过 cc-switch 配置 |
...
| 工具 | 对应教材 Volume | 对应 MetaKing-Skill 组件 | 路由模式 |
|------|----------------|---------------------|---------|
...
| **Trae (MetaKing Agent IDE)** | AG-07 2.5 | L0→L3 四层路由 | SkillHub 注册 + 多层路由 |
