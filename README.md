...
│                              │                                   │
│  ◉ 核心教材（5 Phase × 22 Vol）│  ◉ 直觉引擎（10 个比喻原型）         │
...
| 小节 | 核心内容 | 💡 直觉入口 | 拓扑性展开 |
|------|---------|------------|-----------|
| **2.1 张量的底层内存映射：Strides** | shape=[3,4,5], stride=[20,5,1] 的内存寻址公式；`.view()` 与 `transpose()` 不复制数据只改 stride | Stride = 在内存中"走一步跨多远"；3D 张量 = 多个表格叠起来的立体本子 | → **大教材**：Vol 03 向量空间模型（Strude 是 Embedding 的物理基础）、Vol 04 图数据库 CSR（同源的内存布局思想）<br>→ **手册**：Concept_06 图书馆管理员<br>→ **King-Skill**：Agent 视野隔离（内存隔离的架构级复用） |
...
> ★ **高密度枢纽概念**：C05-C08 每个链接 3-5 个教材 Volume，是跨 Phase 的核心认知枢纽
...
**不怕。** 因为每卷有三层结构（教授说→原书→TA 说），而且有配套的"手册"（10 个比喻原型）提前帮你建立直觉。
...
| 开发 AI Agent | AG-02, AG-03, Vol 08 | Agent 模板 + 编排 + 部署 |
| 做本地推理 | AG-05 | 硬件选型 + 量化 + 蒸馏 |
| 优化 Token 成本 | AG-06 | 缓存体系 + Token 压缩策略 |
| 多 Agent 协作 | AG-03, AG-06, Vol 12 | 通信 + 编排 + ZK 信任 |
| 端侧部署 | AG-04 | 具身智能 + 传感器融合 |
| 理解模型内部 | Vol 06, Vol 11 | 流形假设 + SAE 可解释性 |
...
| 多 Agent 通信范式 | AG-03 §3.3-3.4, Vol 08 §8.3, Vol 12 | 20+ Agent 集群, 呼吸管道 |
...
- **手册（核心概念降维缓存）**：10 个比喻原型，快速建立"是什么"——直觉、类比、映射
...
这套教材的核心设计理念之一就是"先建立系统 1 直觉，再深入系统 2 分析"。10 个比喻原型是精心设计的认知锚点，在读到对应的教材 Volume 时，你会发现这些比喻让复杂概念变得"理所当然"。
...
**路径 A · 新手系统学习（推荐）**
```
Phase 1 (Vol 01→02→02.5→09) → Phase 2 (03→04→04.5) → Phase 3 (AG-01→AG-05→AG-02→AG-06→AG-03→AG-04→Vol 08→08.5) → Phase 4 (05→06→10) → Phase 5 (07→11→12)
```
